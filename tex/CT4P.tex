...
\begin{rem}[Explanation]
	...
	Analogously, a \textit{functor} from a category $\CC$ to a category $\DD$ maps every objects of $\CC$, to an object in $\DD$.
	...
	However, a category does not only consists of a set of objects.
	(Indeed, between every two objects $c_1, c_2 \in \Ob{\CC}$, we have a set of morphisms; denoted $\CHom{\CC}{x}{y}$).
	Hence, a functor, as a transformation between categories, should also map every morphism in $\CC$, to a (suitably typed) morphism in $\DD$.
	...
	(Why? Because between fixed categories, there may be many, or none, functors.)
	...
	These properties are not defined by the user. Nonetheless, they remain to be checked.
