...
\begin{exa}[The category of Haskell types]\label{example:hask}
  Analogously to \cref{exa:coq-cat}, we would like to organize Haskell types and
  Haskell functions into a category.
  However, in Haskell the situation is more complicated than in Coq.
  Due to general recursion, every Haskell type contains the value |undefined|
  (a non-terminating computation); consider the following two functions:
\begin{lstlisting}
undef1 :: a -> a
undef1 = undefined
...
undef2 = \x -> undefined
\end{lstlisting}
  These two programs are different: evaluating |undef1| does not terminate,
  whereas |undef2| immediately returns a function (each of whose applications
  fails to terminate).
  But we have $\Id \Comp$ |undef1| $=$ |undef2|, so in a category of types and
  functions, the unit law forces |undef1| $=$ |undef2|.
  Hence Haskell types and Haskell programs, with equality of programs, do
  \emph{not} form a category.

  A first repair is to consider functions equal when they are \textbf{pointwise}
  equal, that is, to take as morphisms from $X$ to $Y$ equivalence classes of
  functions under
  \[ f \sim g :\iff \forall x : X,\; f(x) = g(x). \]
  This identifies |undef1| and |undef2|.
  But it raises a new question: what does it mean for two \emph{values} $f(x)$
  and $g(x)$ to be equal, when values may themselves be partial --- think of two
  non-terminating computations, or of two infinite lists?

  \emph{Domain theory} gives a precise answer.
  A \textbf{directed-complete partial order (dcpo)} is a partially ordered set
  $(D, \sqsubseteq)$ in which every directed subset $A \subseteq D$ (a non-empty
  subset such that any two of its elements have an upper bound in $A$) has a
  least upper bound $\bigsqcup A$; it is \textbf{pointed} if it has a least
  element $\bot$.
  A function between pointed dcpos is \textbf{(Scott-)continuous} if it is
  monotone and preserves least upper bounds of directed subsets.
  We write $\DCPO$ for the category whose objects are pointed dcpos and whose
  morphisms are continuous functions; identity and composition are those of
  functions (see \cref{exer:dcpo-cat}).

  The category $\DCPO$ is a good category for modelling Haskell types:
  \begin{itemize}
  \item A Haskell type $T$ is interpreted as a pointed dcpo
    $\llbracket T \rrbracket$: the order $\sqsubseteq$ compares values by their
    \emph{information content}, and the least element $\bot$ represents a
    non-terminating computation.
    For instance, |Bool| is interpreted as the ``flat'' dcpo
    $\{\bot, \mathsf{True}, \mathsf{False}\}$ with
    $\bot \sqsubseteq \mathsf{True}$ and $\bot \sqsubseteq \mathsf{False}$,
    and $\mathsf{True}$, $\mathsf{False}$ incomparable.
  \item The function type |a -> b| is interpreted as the set of continuous
    functions from $\llbracket$|a|$\rrbracket$ to $\llbracket$|b|$\rrbracket$,
    ordered pointwise; this is again a pointed dcpo.
  \item Continuity captures computation on infinite data: the output of a
    program on an infinite input (say, an infinite list) is the least upper
    bound of its outputs on the finite approximations of that input.
  \item Recursive definitions are interpreted by \emph{least fixed points}:
    every continuous $f : D \to D$ on a pointed dcpo has a least fixed point,
    given by $\bigsqcup_{n\in\NN} f^{n}(\bot)$.
    This interprets general recursion --- precisely the language feature that
    introduced $\bot$ in the first place.
  \end{itemize}
  We therefore \emph{define} $\HASK$ to be the category $\DCPO$, with Haskell
  types and programs interpreted as its objects and morphisms.
  In this interpretation, |undef1| and |undef2| denote the \emph{same} morphism
  --- the least element $\lambda x.\bot$ of the function space --- so the
  identification forced by the category axioms is validated by the model.
  (The price is that $\DCPO$ does not model Haskell's |seq| primitive, which
  distinguishes |undef1| from |undef2|; modelling |seq| faithfully requires a
  more refined category.)
\end{exa}

\begin{exer}\label{exer:dcpo-cat}
  Show that $\DCPO$ is indeed a category: show that identity functions are
  continuous, and that the composition of two continuous functions is
  continuous.
\end{exer}

\begin{reading*}
  The category $\HASK$ of Haskell types is discussed on the Haskell wiki
  \cite{haskell-wiki-hask}.
  An introduction to domain theory is given in the handbook chapter by Abramsky
  and Jung \cite{abramsky-jung}.
  Danielsson et al.\ \cite{DBLP:conf/popl/DanielssonHJG06} justify why it is
  nevertheless ``morally correct'' to reason about Haskell programs as if
  Haskell types formed a category of sets and total functions.
\end{reading*}
...
\begin{exa}[Divisibility]\label{exa:divisibility}
  For natural numbers $m, n \in \NN$, we say that $m$ \textbf{divides} $n$,
  written $m \mid n$, if there is some $k \in \NN$ with $n = k \cdot m$.
  The relation $\mid$ is reflexive and transitive, so $(\NN, \mid)$ is a
  preordered set; it is even a poset, since $m \mid n$ and $n \mid m$ imply
  $m = n$.
  We hence obtain a category $\POS(\NN, \mid)$: its objects are the natural
  numbers, and there is a (unique) morphism $m \to n$ if and only if $m$
  divides $n$.
  Note that $1$ divides every natural number, and that every natural number
  divides $0$.
\end{exa}

\begin{exa}[Truth values]\label{exa:truthvalues}
  Let $\mathbb{B} := \{\mathsf{false}, \mathsf{true}\}$, ordered by
  $\mathsf{false} \leq \mathsf{true}$.
  The associated category $\POS(\mathbb{B}, \leq)$ has two objects and, besides
  the identities, exactly one morphism $\mathsf{false} \to \mathsf{true}$;
  it is the interval category of \cref{exa:graph_intervalcat}.
  It is fruitful to think of a morphism $P \to Q$ as ``$P$ implies $Q$'':
  falsehood implies truth, but not conversely.
  More generally, logical propositions, preordered by implication, form a
  category.
\end{exa}
...
\begin{exer}[\cref{sol:initial_dcpo}]\label{exer:initial_dcpo}
  Does the category $\DCPO$ (defined in \cref{example:hask}) have an initial object?

  Hint: constant functions are continuous.
\end{exer}

\begin{exer}[\cref{sol:initial_divisibility}]\label{exer:initial_divisibility}
  Identify an initial object in the divisibility category $\POS(\NN, \mid)$ of
  \cref{exa:divisibility}, and in the category $\POS(\mathbb{B}, \leq)$ of
  truth values of \cref{exa:truthvalues}.
\end{exer}
...
\begin{exer}[\cref{sol:terminal_dcpo}]\label{exer:terminal_dcpo}
  Identify a terminal object in the category $\DCPO$ and show that it is indeed terminal.
\end{exer}

\begin{exer}[\cref{sol:terminal_divisibility}]\label{exer:terminal_divisibility}
  Identify a terminal object in the divisibility category $\POS(\NN, \mid)$,
  and in the category $\POS(\mathbb{B}, \leq)$ of truth values.
\end{exer}

\begin{dfn}\label{dfn:zero-object}
  Let $\CC$ be a category. An object $Z \in \Ob{\CC}$ is a \textbf{zero object}
  if it is both initial and terminal.
\end{dfn}

\begin{exer}[\cref{sol:zero_set}]\label{exer:zero_set}
  Does the category $\SET$ have a zero object?
\end{exer}

\begin{exer}[\cref{sol:zero_ptset}]\label{exer:zero_ptset}
  Show that the category $\PTSET$ of pointed sets has a zero object.
  (For the initial object, see \cref{exer:initial_pointset}.)
\end{exer}

\begin{exer}[\cref{sol:zero_mon}]\label{exer:zero_mon}
  Show that the trivial monoid $(\{e\}, m, e)$ --- with the only possible
  multiplication --- is a zero object in the category $\MON$ of monoids.
\end{exer}
...
\begin{exer}[\cref{sol:product_dcpo}]\label{exer:product_dcpo}
  Let $D$ and $E$ be pointed dcpos.
  Show that the cartesian product $D \times E$, ordered componentwise, i.e.
  \[ (d_1,e_1) \sqsubseteq (d_2,e_2) :\iff d_1 \sqsubseteq_D d_2 \text{ and } e_1 \sqsubseteq_E e_2, \]
  is a pointed dcpo, and that it is a product of $D$ and $E$ in $\DCPO$.
\end{exer}

\begin{exer}[\cref{sol:product_divisibility}]\label{exer:product_divisibility}
  Describe products in the divisibility category $\POS(\NN, \mid)$, and in the
  category $\POS(\mathbb{B}, \leq)$ of truth values.

  Hint: by \cref{exer:product_posetcat}, a product in a poset is a greatest
  lower bound.
\end{exer}

\begin{exer}[\cref{sol:product_ptset}]\label{exer:product_ptset}
  Let $(X, x_0)$ and $(Y, y_0)$ be pointed sets.
  Construct a product of $(X,x_0)$ and $(Y,y_0)$ in $\PTSET$.
\end{exer}
...
\begin{exer}[\cref{sol:coproduct_dcpo}]\label{exer:coproduct_dcpo}
  Show that the category $\DCPO$ does not have binary coproducts.

  Hint: Let $\mathbb{O} := \{\bot \sqsubseteq \top\}$ be the two-element dcpo.
  Assuming a coproduct $(C, \inl, \inr)$ of pointed dcpos $A$ and $B$ exists,
  first show that $\bot_C$ must lie in the image of one of the injections
  (otherwise cotupling two constant functions into $\mathbb{O}$ does not have a
  \emph{unique} mediating morphism).
  Then derive a contradiction by cotupling a constant-$\top$ and a
  constant-$\bot$ function into $\mathbb{O}$.
\end{exer}

\begin{rem}
  The failure of \cref{exer:coproduct_dcpo} is instructive for the modelling of
  Haskell's |Either a b|.
  The natural candidate is the \emph{separated sum} $(D + E)_\bot$: the disjoint
  union of $D$ and $E$ with a new least element $\bot$ adjoined --- this matches
  Haskell, where the values of |Either a b| are |Left x|, |Right y|, and the
  non-terminating computation.
  The separated sum admits injections and, for all $f : D \to C$ and
  $g : E \to C$, a mediating morphism $h$ with $\co{\inl}{h} = f$ and
  $\co{\inr}{h} = g$ --- but $h$ is not unique, since $h(\bot)$ may be any
  element below the images: it is only a \emph{weak} coproduct.
  In the subcategory of $\DCPO$ of \emph{strict} continuous functions (those
  preserving $\bot$), the \emph{coalesced sum} --- the disjoint union with the
  two least elements identified --- is a genuine coproduct, and the one-point
  dcpo becomes an initial object.
\end{rem}

\begin{exer}[\cref{sol:coproduct_divisibility}]\label{exer:coproduct_divisibility}
  Describe coproducts in the divisibility category $\POS(\NN, \mid)$, and in
  the category $\POS(\mathbb{B}, \leq)$ of truth values.
\end{exer}

\begin{exer}[\cref{sol:coproduct_ptset}]\label{exer:coproduct_ptset}
  Let $(X, x_0)$ and $(Y, y_0)$ be pointed sets.
  Construct a coproduct of $(X,x_0)$ and $(Y,y_0)$ in $\PTSET$.

  Hint: in the disjoint union $X + Y$, the two base points must become equal.
  The resulting construction is called the \textbf{wedge sum};
  compare it with the coalesced sum of pointed dcpos discussed after
  \cref{exer:coproduct_dcpo}.
\end{exer}

\begin{rem}
  \Cref{exer:product_divisibility,exer:coproduct_divisibility} show that
  universal properties subsume classical constructions:
  in $\POS(\NN,\mid)$, the product is the greatest common divisor and the
  coproduct is the least common multiple;
  in the category of truth values, the product is conjunction and the
  coproduct is disjunction.
\end{rem}
...
\begin{exer}[\cref{sol:initialalg_node_btree}]\label{exer:initialalg_node_btree}
  Let $A$ be a set and define $F_A$ to be the functor induced by
  \[
  F_A : \SET \to \SET : X \mapsto 1 + (X \times A \times X).
  \]
  Show that the initial $F_A$-algebra is given by the set $\mathsf{Tree}(A)$ of
  binary trees carrying an element of $A$ at each \emph{internal node} (and
  nothing at the leaves), with constructors
  $\mathsf{leaf} : 1 \to \mathsf{Tree}(A)$ and
  $\mathsf{node} : \mathsf{Tree}(A) \times A \times \mathsf{Tree}(A) \to \mathsf{Tree}(A)$.

  In Haskell, this datatype is given by:
\begin{lstlisting}
data Tree a = Leaf | Node (Tree a) a (Tree a)
\end{lstlisting}
  Compare with \cref{exer:initialalg_for_btree}, where the labels sit at the
  leaves instead.
\end{exer}

\begin{exer}[\cref{sol:initialalg_rose}]\label{exer:initialalg_rose}
  Let $A$ be a set and define $F_A$ to be the functor induced by
  \[
  F_A : \SET \to \SET : X \mapsto A \times \List(X).
  \]
  Note that $F_A$ is a composition of functors we know: the list functor of
  \cref{example:functor_list} followed by the functor $(A \times -)$ of
  \cref{exer:functor_prod_on_left}.

  Show that the initial $F_A$-algebra is given by the set $\mathsf{Rose}(A)$ of
  \emph{rose trees}: finitely branching trees with an element of $A$ at each
  node, with constructor
  $\mathsf{node} : A \times \List(\mathsf{Rose}(A)) \to \mathsf{Rose}(A)$.
...
\begin{exa}[Moore machines]\label{exa:moore}
  Fix sets $A$ (of \emph{inputs}) and $B$ (of \emph{outputs}), and consider the
  functor given on objects by
  \[
  F(X) := B \times (A \to X),
  \]
  a combination of the functors $(B \times -)$ and $(A \to)$ from
  \cref{sec:functors}.
  An $F$-coalgebra $\langle \mathsf{out}, \mathsf{next}\rangle : X \to B \times (A \to X)$
  is precisely a \textbf{Moore machine}: a set $X$ of states, a function
  $\mathsf{out} : X \to B$ assigning to every state an observable output, and a
  transition function $\mathsf{next} : X \to (A \to X)$ assigning to every
  state $x$ and input $a$ a successor state $\mathsf{next}(x)(a)$.

  For $A = 1$, we have $F(X) \cong B \times X$, and we recover streams over
  $B$: a Moore machine with trivial input is a stream of outputs.
\end{exa}

\begin{exer}[\cref{sol:moore_terminal}]\label{exer:moore_terminal}
  Let $A^{*}$ denote the set of finite words over $A$ (that is, of lists over
  $A$); we write $\epsilon$ for the empty word and $aw$ for the word $w$
  prepended with $a \in A$.
  Show that the terminal $F$-coalgebra for the functor $F$ of \cref{exa:moore}
  is given by the set $A^{*} \to B$ of \emph{behaviours}, with structure map
  \[
  \phi \mapsto \big(\phi(\epsilon),\; \lambda a. \lambda w. \phi(aw)\big).
  \]
  What does the anamorphism of a Moore machine
  $(X, \langle \mathsf{out}, \mathsf{next}\rangle)$ compute?
\end{exer}
...
\begin{solution}[\cref{exer:coproduct_dcpo}]\label{sol:coproduct_dcpo}
  Suppose, for contradiction, that pointed dcpos $A$ and $B$ have a coproduct
  $(C, \inl, \inr)$ in $\DCPO$.
  Write $\mathbb{O} = \{\bot \sqsubseteq \top\}$.

  \textbf{Step 1: $\bot_C$ lies in the image of $\inl$ or of $\inr$.}
  Suppose not.
  Consider the constant functions $\const_\top : A \to \mathbb{O}$ and
  $\const_\top : B \to \mathbb{O}$.
  The constant function $h = \const_\top : C \to \mathbb{O}$ satisfies
  $\co{\inl}{h} = \const_\top$ and $\co{\inr}{h} = \const_\top$.
  But so does the function $h' : C \to \mathbb{O}$ with $h'(\bot_C) := \bot$ and
  $h'(c) := \top$ for $c \neq \bot_C$:
  it agrees with $h$ on the images of the injections by assumption;
  it is monotone since $\bot_C$ is the least element of $C$;
  and it preserves directed least upper bounds, since the only directed subset
  of $C$ with least upper bound $\bot_C$ is $\{\bot_C\}$.
  Since $h \neq h'$, this contradicts the uniqueness in the universal property.

  \textbf{Step 2: deriving the contradiction.}
  By Step 1 we may assume $\bot_C = \inl(a_0)$ for some $a_0 \in A$ (the other
  case is symmetric).
  Consider $f := \const_\top : A \to \mathbb{O}$ and
  $g := \const_\bot : B \to \mathbb{O}$.
  Any morphism $h : C \to \mathbb{O}$ with $\co{\inl}{h} = f$ satisfies
  $h(\bot_C) = h(\inl(a_0)) = \top$, and hence, by monotonicity, $h(c) = \top$
  for \emph{every} $c \in C$.
  But then $\co{\inr}{h} = \const_\top \neq g$, since $B$ is non-empty.
  So no mediating morphism for $(f,g)$ exists --- contradicting the universal
  property.
  Hence no two pointed dcpos have a coproduct in $\DCPO$.
\end{solution}

\begin{solution}[\cref{exer:coproduct_divisibility}]\label{sol:coproduct_divisibility}
  Dually to \cref{sol:product_divisibility}, a coproduct in a poset category is
  a least upper bound.
  In $\POS(\NN,\mid)$, an upper bound of $m$ and $n$ is a common multiple, and
  the least common multiple $\operatorname{lcm}(m,n)$ is the common multiple
  dividing every other common multiple; hence the coproduct of $m$ and $n$ is
  $\operatorname{lcm}(m,n)$.
  In $\POS(\mathbb{B},\leq)$, the least upper bound of $P$ and $Q$ is the
  disjunction $P \vee Q$.
\end{solution}

\begin{solution}[\cref{exer:coproduct_ptset}]\label{sol:coproduct_ptset}
  Define the \emph{wedge sum} $(X,x_0) \vee (Y,y_0)$ as the quotient of the
  disjoint union $X + Y$ by the identification $\inl(x_0) = \inr(y_0)$, pointed
  by this identified element; write $[z]$ for the class of $z \in X + Y$.
  The functions $x \mapsto [\inl(x)]$ and $y \mapsto [\inr(y)]$ are morphisms of
  pointed sets.
  Given morphisms $f : (X,x_0) \to (Z,z_0)$ and $g : (Y,y_0) \to (Z,z_0)$, the
  cotupling $[f,g] : X + Y \to Z$ of \cref{exer:coproduct_set} satisfies
  $[f,g](\inl(x_0)) = f(x_0) = z_0 = g(y_0) = [f,g](\inr(y_0))$, so it descends
  to a function on the wedge sum; this function is a morphism of pointed sets
  and makes the coproduct triangles commute.
  Uniqueness follows from uniqueness of $[f,g]$ in $\SET$, since the quotient
  map $X + Y \to (X,x_0) \vee (Y,y_0)$ is surjective.
\end{solution}
...
\begin{solution}[\cref{exer:initial_dcpo}]\label{sol:initial_dcpo}
  The category $\DCPO$ does not have an initial object.
  First, note that every pointed dcpo is non-empty, since it contains $\bot$.
  Now let $I$ be any pointed dcpo.
  For every pointed dcpo $D$ and every element $d \in D$, the constant function
  $\const_d : I \to D$ is continuous.
  Hence $\CHom{\DCPO}{I}{D}$ has at least as many elements as $D$.
  Choosing $D$ with at least two elements, e.g., $\mathbb{O} = \{\bot \sqsubseteq \top\}$,
  we see that $\CHom{\DCPO}{I}{D}$ is not a singleton, so $I$ is not initial.
\end{solution}

\begin{solution}[\cref{exer:initial_divisibility}]\label{sol:initial_divisibility}
  By \cref{sol:initial_posetcat}, an initial object in a poset category is a
  least element.
  In $\POS(\NN,\mid)$, the least element with respect to divisibility is $1$,
  since $1 \mid n$ for every $n \in \NN$.
  In $\POS(\mathbb{B},\leq)$, the least element is $\mathsf{false}$:
  ``ex falso quodlibet''.
\end{solution}
...
\begin{solution}[\cref{exer:terminal_dcpo}]\label{sol:terminal_dcpo}
  A terminal object is the one-point dcpo $\{\bot\}$.
  For any pointed dcpo $D$ there is exactly one function $D \to \{\bot\}$, and it
  is continuous: it is monotone, and it preserves least upper bounds of directed
  subsets since $\{\bot\}$ has only one element.
\end{solution}

\begin{solution}[\cref{exer:terminal_divisibility}]\label{sol:terminal_divisibility}
  By \cref{sol:terminal_posetcat}, a terminal object in a poset category is a
  greatest element.
  In $\POS(\NN,\mid)$, the greatest element with respect to divisibility is
  $0$, since $n \mid 0$ for every $n \in \NN$.
  In $\POS(\mathbb{B},\leq)$, the greatest element is $\mathsf{true}$:
  everything implies truth.
\end{solution}

\begin{solution}[\cref{exer:zero_set}]\label{sol:zero_set}
  No. The initial object of $\SET$ is the empty set, and the terminal objects
  are the one-element sets (\cref{exer:initial_set,exer:terminal_set}).
  The empty set is not terminal, since there is no function
  $\{\star\} \to \emptyset$ at all; and a one-element set is not initial, since,
  e.g., there are two functions $\{\star\} \to \Bool$.
  Since initial and terminal objects are unique up to isomorphism, no set can
  be both.
\end{solution}

\begin{solution}[\cref{exer:zero_ptset}]\label{sol:zero_ptset}
  By \cref{sol:initial_pointset}, $(\{\star\},\star)$ is an initial object of
  $\PTSET$.
  It is also terminal: for any pointed set $(X,x)$, a morphism
  $(X,x) \to (\{\star\},\star)$ is a function $X \to \{\star\}$ mapping $x$ to
  $\star$; there is exactly one function $X \to \{\star\}$, and it maps $x$ to
  $\star$.
  Hence $(\{\star\},\star)$ is a zero object.
\end{solution}

\begin{solution}[\cref{exer:zero_mon}]\label{sol:zero_mon}
  Let $(M, m_M, e_M)$ be a monoid.
  A morphism of monoids $(\{e\},m,e) \to (M,m_M,e_M)$ must map $e$ to $e_M$,
  and the function $e \mapsto e_M$ is indeed a morphism of monoids; hence the
  trivial monoid is initial.
  Conversely, there is exactly one function $M \to \{e\}$; it preserves the
  multiplication and the identity element, so it is the unique morphism of
  monoids $(M,m_M,e_M) \to (\{e\},m,e)$; hence the trivial monoid is terminal.
\end{solution}
...
\begin{solution}[\cref{exer:product_dcpo}]\label{sol:product_dcpo}
  The componentwise order makes $D \times E$ a partially ordered set with least
  element $(\bot_D, \bot_E)$.
  If $A \subseteq D \times E$ is directed, then so are its projections
  $A_1 \subseteq D$ and $A_2 \subseteq E$, and one checks that
  $\bigsqcup A = (\bigsqcup A_1, \bigsqcup A_2)$.
  Hence $D \times E$ is a pointed dcpo, and the projections $\projl, \projr$
  preserve directed least upper bounds, i.e., they are continuous.

  Given continuous functions $q_1 : Q \to D$ and $q_2 : Q \to E$, the unique
  function $f : Q \to D \times E$ with $\co{f}{\projl} = q_1$ and
  $\co{f}{\projr} = q_2$ is $f(q) := (q_1(q), q_2(q))$, exactly as in $\SET$
  (\cref{sol:product_set}); it remains to observe that $f$ is continuous, which
  follows since least upper bounds in $D \times E$ are computed componentwise.
  Uniqueness follows as in $\SET$, since a function into $D\times E$ is
  determined by its two components.
\end{solution}

\begin{solution}[\cref{exer:product_divisibility}]\label{sol:product_divisibility}
  By \cref{exer:product_posetcat}, a product of $m$ and $n$ in a poset category
  is a greatest lower bound of $m$ and $n$.
  In $\POS(\NN,\mid)$, a lower bound of $m$ and $n$ is a common divisor, and
  the greatest common divisor $\gcd(m,n)$ is precisely the common divisor that
  is divided by every other common divisor.
  Hence the product of $m$ and $n$ is $\gcd(m,n)$.
  In $\POS(\mathbb{B},\leq)$, the greatest lower bound of $P$ and $Q$ is the
  conjunction $P \wedge Q$: it implies both $P$ and $Q$, and any truth value
  implying both $P$ and $Q$ implies $P \wedge Q$.
\end{solution}

\begin{solution}[\cref{exer:product_ptset}]\label{sol:product_ptset}
  A product of $(X,x_0)$ and $(Y,y_0)$ is the pointed set
  $(X \times Y, (x_0,y_0))$, together with the projections $\projl$ and
  $\projr$, which are morphisms of pointed sets since they map $(x_0,y_0)$ to
  $x_0$ and $y_0$, respectively.
  Given morphisms $q_1 : (Q,q_0) \to (X,x_0)$ and $q_2 : (Q,q_0) \to (Y,y_0)$,
  the unique mediating function is $q \mapsto (q_1(q), q_2(q))$, as in $\SET$
  (\cref{sol:product_set}); it maps $q_0$ to $(x_0,y_0)$, so it is a morphism
  of pointed sets, and uniqueness follows as in $\SET$.
\end{solution}
...
\begin{solution}[\cref{exer:initialalg_node_btree}]\label{sol:initialalg_node_btree}
  An $F_A$-algebra consists of a set $X$, an element $x_0 \in X$
  (the image of the unique element of $1$), and a function
  $\phi : X \times A \times X \to X$.
  Given such an algebra, define $h : \mathsf{Tree}(A) \to X$ by recursion:
  \[
  h(\mathsf{leaf}) := x_0,
  \qquad
  h(\mathsf{node}(l, a, r)) := \phi(h(l), a, h(r)).
  \]
  By construction, $h$ is a morphism of $F_A$-algebras.
  For uniqueness, let $g$ be another morphism of algebras from
  $(\mathsf{Tree}(A), [\mathsf{leaf}, \mathsf{node}])$ to $(X, [x_0, \phi])$;
  then $g(\mathsf{leaf}) = x_0$ and
  $g(\mathsf{node}(l,a,r)) = \phi(g(l), a, g(r))$, so $g = h$ follows by
  structural induction on trees.
\end{solution}

\begin{solution}[\cref{exer:initialalg_rose}]\label{sol:initialalg_rose}
  An $F_A$-algebra consists of a set $X$ and a function
  $\phi : A \times \List(X) \to X$.
  Given such an algebra, define $h : \mathsf{Rose}(A) \to X$ by recursion:
  \[
  h(\mathsf{node}(a, ts)) := \phi\big(a, \List(h)(ts)\big),
  \]
  where $\List(h)$ applies $h$ to every tree in the list $ts$ of subtrees.
  By construction, $h$ is a morphism of $F_A$-algebras, and any morphism of
  algebras satisfies the same equation, so uniqueness follows by structural
  induction on rose trees (using that a rose tree is strictly larger than each
  of its subtrees).
\end{solution}
...
\begin{solution}[\cref{exer:moore_terminal}]\label{sol:moore_terminal}
  Write $\mathsf{Beh} := (A^{*} \to B)$ and
  $\Out : \mathsf{Beh} \to B \times (A \to \mathsf{Beh})$ for the structure map
  $\Out(\phi) := (\phi(\epsilon), \lambda a. \lambda w. \phi(aw))$.

  Let $(X, \langle \mathsf{out}, \mathsf{next}\rangle)$ be an $F$-coalgebra.
  Define $\mathsf{beh} : X \to \mathsf{Beh}$ by recursion on the length of
  words:
  \[
  \mathsf{beh}(x)(\epsilon) := \mathsf{out}(x),
  \qquad
  \mathsf{beh}(x)(aw) := \mathsf{beh}\big(\mathsf{next}(x)(a)\big)(w).
  \]
  Then $\mathsf{beh}$ is a morphism of coalgebras: the first component of
  $\Out(\mathsf{beh}(x))$ is $\mathsf{beh}(x)(\epsilon) = \mathsf{out}(x)$, and
  for every $a \in A$, we have
  $\lambda w. \mathsf{beh}(x)(aw) = \mathsf{beh}(\mathsf{next}(x)(a))$ by
  definition.

  For uniqueness, let $g : X \to \mathsf{Beh}$ be any morphism of coalgebras.
  The two components of the coalgebra morphism square say precisely that
  $g(x)(\epsilon) = \mathsf{out}(x)$ and
  $g(x)(aw) = g(\mathsf{next}(x)(a))(w)$; these equations determine $g$
  uniquely by induction on the length of words, so $g = \mathsf{beh}$.

  The anamorphism of a Moore machine therefore assigns to every state its
  \emph{behaviour}: the function mapping each input word to the output
  observed after processing that word starting in the given state.
\end{solution}
