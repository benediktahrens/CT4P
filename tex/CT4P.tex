...
\begin{exer}[\cref{sol:categories_coming_from_monoids}]\label{exer:categories_coming_from_monoids}
  Let $\CC$ be a category. When does $\CC$ ``come from a monoid'', that is, when is there a monoid $(M,m,e)$ such that $\C$ of the form $\MONtoCAT(M,m,e)$?
\end{exer}
...
\begin{exer}
  ...
  \begin{enumerate}
  \item The category that looks like this: $0 \to 1 \to 2 \to \ldots$. (It is the category generated by the preorder on natural numbers given by the ``less than or equal'' relation.)
