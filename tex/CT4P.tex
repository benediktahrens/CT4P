...
\item [Implication] If $X$ is a set and $P$ and $Q$ are predicates on $X$, we write $P\implies Q$ to express that if $P(x)$ holds for an element $x\in X$, then also $Q(x)$  holds for the element $x$. Moreover, we write $P \iff Q$ if $P\implies Q$ and $Q\implies P$.
