...
\item For each object $X\in \Ob{\CC}$, a morphism $\Id[X] \in \Hom[C]{X}{Y}$, called the \textit{identity morphism} on $X$.
...
\begin{exer} Is $\POS$ a preorder-category itself? That is, is there at most one morphism between any two objects?
\end{exer}
...
\begin{exer}\label{monoidcategory}
...
\end{itemize}
Show that $\MON(M,m,e)$ is indeed a category.
\end{exer}
...
In fact, we can associate a different number of categories to a single monad. We can for example consider an arbitrary set of objects $I$ and the defining the hom-sets as follows:
\[
\Hom{i}{j} := 
\begin{cases}
M ,\quad \text{ if } i=j,\\
...
\begin{exa} Consider the monoid $(\mathbb{Q}, \times, 1)$ (where $\times$ is the usual multiplication of rational numbers). By definition of $\MON(M,m,e)$, we have that $\MON(\mathbb{Q},\times, 1)$ is the category which has a unique object and each morphism correspond with a rational number $q\in\mathbb{Q}$.
\end{exa}

\begin{exer}
\begin{enumerate}
\item When is a category $\CC$ of the form $\MON(M,m,e)$?
\item Define a category $\MON$ whose objects are monoids, i.e. define a suitable notion of morphism between monoids and moreover show that this indeed defines a category.
\end{enumerate}
\end{exer}

\begin{exer} Define a category $\CC$ whose objects are the natural numbers (i.e. $\Ob{\CC} = \mathbb{N}$) and whose hom-sets $\CHom{\CC}{n}{m}$ are given by the $(n\times m)$-matrices.
\end{exer}
...
\begin{itemize}
\item $\Hom{x}{x} = \left\{\Id{x}, \co{f}{g}, (\co{f}{g})^2, \co{f}{g}^3, \cdots\right\}$, 
\item $\Hom{y}{y} = \left\{\Id{y}, \co{g}{f}, (\co{g}{f})^2, \co{g}{f}^3, \cdots\right\}$, 
\item $\Hom{x}{y} = \left\{f, \co{f}{(\co{g}{f})}, \co{f}{(\co{g}{f})^2}, \co{f}{(\co{g}{f})^3}, \cdots\right\}$, 
\item $\Hom{y}{x} = \left\{g, \co{g}{(\co{f}{g})}, \co{g}{(\co{f}{g})^2}, \co{g}{(\co{f}{g})^3}, \cdots\right\}$, 
\end{itemize}
\end{itemize}
\end{exa}
...
\begin{exer} Can you characterize the isomorphisms in $\SET$?
\end{exer}

\begin{exer} Can you characterize the isomorphisms in $\POS$?
\end{exer}

\begin{exer} Let $(X,\leq)$ be a poset. Can you characterize the isomorphisms in $\POS(X,\leq)$?
\end{exer}
...
\begin{dfn}[Monomorphism]
  Let $f : a \to b$ be a morphism in $\CC$. We say that $f$ is a \textbf{monomorphism} if, for any two morphisms $g_1, g_2 : z \to a$, we have
...
\begin{dfn}[Epi]
  Let $f : a \to b$ be a morphism in $\CC$. We say that $f$ is an \textbf{epimorphism} if, for any two morphisms $g_1, g_2 : b \to z$, we have
...
\begin{exer}
  In the category of sets, show that any section is injective.
  Hint: you can use \cref{ex:mono-inj}.
\end{exer}
\begin{exer}
  In the category of sets, show that any retraction is surjective.
...
\begin{exer} Show that the converse of \cref{exer:iso_to_monoepi} does not hold in general, i.e. give an example of a category where there exists a morphism which is both an epi- and a monomorphism, but which is not an isomorphism.\\
...
\begin{exer}\label{ex:monoid_functors} Let $(M,m,e)$ be a monad and let $\MON(M,m,e)$ be its corresponding category as defined in \cref{monoidcategory}. Can you characterize/describe the functors from $\MON(M,m,e)$ to $\SET$?
\end{exer}
