...
Natural transformations could be said to be the heart of category theory.
Category theory was originally introduced developed to express the notion of a natural transformation.
In particular, many of the constructions seen before and after can be expressed in terms of such transformations.
When a category is considered as a model for a programming language, natural transformations are used to interpret polymorphism.

\begin{paragraph}{Motivation}
From monads, to natural transformations, representable functors and colimits and adjunctions, and Yoneda lemma.
\end{paragraph}
...
%$a \mapsto \CC(a, x) \times \CC(a, y)$.
\end{enumerate}
\end{exer}

Recall that $\MON$ denotes the category of monoids and monoid homomorphisms.
\begin{exer}
\label{exer:forgetful-functor-is-representable}
...
\end{exer}
\begin{solution}
Observe that the representability of $U$ is a special case of the furthermore statement, indeed: every set $A$ is isomorphic to the set $\bullet \to A$ of functions from the singleton/unit set into $A$.
...
\textbf{Injectivity}. Let $g, h \in \List(A)$ and assume $\phi_M(g) = \phi_M(h)$.
...
Let $g' \in \SET(A, UM)$.
We have to construct a $g \in \MON(\List(A), M)$ such that $g' = \phi_M(g) := g \circ \eta_A$, i.e., $\forall a \in A, g'(a) = g([a])$.
It is easy that $g$ can be defined as a fold from the which the monoid structure on $g$ follows.
...
\end{exer}
\begin{solution}
Recall that a downward closed subset of $L$ is given by a subset $S$ of $L$ which is closed under the following property:
\[
\forall x \in S, \forall a \in L, a \leq x \Rightarrow a \in S.
\]
We claim that the representable functors correspond to the downward closed subsets of $\mathcal{L}$ with a maximal element.

Let $F : \op{\mathcal{L}} \to \SET$ be a representable functor.
Then, $F \cong \mathcal{L}(-,x)$ for some $x \in L$.
By definition of $\mathcal{L}$, we have that $F(x)$ is the (po)set of elements in $\mathcal{L}$ which are less then $x$, i.e., $F(x) \cong \{ a \in L \mid a \leq x \}$.
Observe that $F(x)$ contains $x$ as a maximal element.
Conversely, if $D$ is a downward closed subset with a maximal element $x$, we obtain a representable functor $F := \mathcal{L}(-,x)$.
\end{solution}

% Observe that downward closed subsets, form a completion. Why, this poset (as it is a subposet of the powerobject, 
% 

\begin{rem}[From this to free completions and adjunctions, and the role of Yoneda]
The previous exercises illustrate that representable functors, and natural transformations, both generalize and connect many constructions.
Indeed:
\begin{itemize}
\item \cref{exer:coproducts-as-representable} illustrates that universal properties can be described in terms of representables.
\item \cref{exer:representables-on-posets} 
\item \cref{exer:forgetful-functor-is-representable} illustrates that adjointness can be described in terms of representables.
\end{itemize}
