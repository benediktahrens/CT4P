...
\newcommand{\pmap}[2]{ \langle #1, #2 \rangle }  % called \productmap in CT4P
\newcommand{\cmap}[2]{ [ #1, #2 ] }           % called \coproductmap in CT4P
...
To show that $\alpha$ is an isomorphism we have to show that equations \ref{unit1} and \ref{unit2} hold.
\textbf{Claim $\co {\beta} {\alpha} = \Id[\CQ]$}(\cref{unit1})
\begin{proof}
Fix $(a',b') \in \CQ$.
\begin{align*}
...
=&\qquad \text{\{ Definition of $\alpha$ \}} \\
 &\quad (\co {\inl} {\cmap{a'}{b'}}, \co {\inr} {\cmap{a'}{b'}}) \\
=&\qquad \text{\{ Using \ref{ga} and \ref{gb} \}} \\
 &\quad (a', b') \\
...
\textbf{Claim $\co {\alpha} {\beta} = \Id[\CP]$} (\cref{unit2}).
\begin{proof}
Fix $k' \in \CP$.
\begin{align*}
...
=&\qquad \text{\{ Definition of $\beta$ \}} \\
 &\quad \cmap{\co {\inl} {k'}}{\co {\inr} {k'}}\\
=&\qquad \text{\{ Uniqueness of $\cmap{\co {\inl} {k'}}{\co {\inr} {k'}} \in \CC(A+B,X) \}$} \\
 &\quad k'\\
...
\[\alpha(k):=(\co {k} {\projl}, \co {k} {\projr})\] and \[\beta(a,b) := \pmap{a}{b}\] where $\pmap{a}{b}$, also known as the product map, is the unique morphism that makes the corresponding product triangles commute, as in the following diagram:
...
\begin{itemize}
    ...
    \item $\cmap{a}{b}$ becomes $\pmap{a}{b}$
\end{itemize}
\end{solution}
