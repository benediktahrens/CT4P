...
\begin{solution}[\cref{exer:initialalg_for_bifunctor_functor}]\label{sol:initialalg_for_bifunctor_functor}
	...
	Then $ (\Initalg{F_B}, \In_B \circ F(\_, \Initalg{F_B})(f): F_A(\Initalg{F_B}) \to \Initalg{F_B}) $ is an $ F_A $-algebra, so by the initiality of $ \Initalg{F_A} $, there exists a unique morphism $ f^\prime: \Initalg{F_A} \to \Initalg{F_B} $ such that 
	...
	\begin{center}
		\begin{tikzcd}
			F(A, \Initalg{F_B}) \arrow[r, "{F(\_, \Initalg{F_B})(f)}"] \arrow[d, "{F(A, \_)(G(g))}"'] & F(B, \Initalg{F_B}) \arrow[d, "{F(B, \_)(G(g))}"]\\
			F(A, \Initalg{F_C}) \arrow[r, "{F(\_, \Initalg{F_C})(f)}"] & F(B, \Initalg{F_C})
		\end{tikzcd}
	\end{center}
