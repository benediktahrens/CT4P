...
\begin{dfn}\label{dfn:initial-alg}
  Let $F:\CC\to\CC$ be an endofunctor. The \textbf{initial $F$-algebra} (if it exists), is the initial object in $\ALG{F}$.
  Unfolding the definition, this means that it is an $F$-algebra $(\mu F, \In)$ such that for any $F$-algebra $(C,\phi)$, there exists a unique morphism $\catam{\phi} \in \CHom{\CC}{\mu F}{C}$ such that the following diagram commutes:
\[
\begin{tikzcd}
{F(\mu F)} 
\arrow[r, "in"] 
...
\begin{exer}
  Let $F:\CC\to\CC$ be an endofunctor and let $(\mu F, \In)$ be an initial algebra. Show that the following properties holds:
\begin{enumerate}
\item $\Id = \catam{\In}$.
\item For $F$-algebras $(C,\phi)$ and $(D,\psi)$ and $f\in\CHom{\CC}{C}{D}$, we have 
\[
\co{f}{\phi} = \co{\psi}{F(f)} \implies \co{f}{\catam{\phi}} = \catam{\psi}.
\]
\end{enumerate} 
\end{exer}
...
\begin{enumerate}
\item Show that an $F_A$-algebra consists of a triple $(X,n,c)$, where $X$ is a set, $n\in X$ is an element of $X$, and $c : A \times X \to X$ is a function.
